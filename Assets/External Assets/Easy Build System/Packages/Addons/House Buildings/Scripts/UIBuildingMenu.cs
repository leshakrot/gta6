...
namespace EasyBuildSystem.Packages.Addons.BuildingMenu
{
    public class UIBuildingMenu : MonoBehaviour
    {
        ...
        [SerializeField] int m_Credit = PlayerBank.instance.GetMoneyAmount();
                ...
                for (int i = 0; i < m_Categories.Length; i++)
                {
                    for (int x = 0; x < m_Categories[i].Items.Length; x++)
                    {
                        ...
                        buildingPart.GetGeneralSettings.Identifier)
                        {
                            ...
                            PlayerBank.instance.RemoveMoney(m_Categories[i].Items[x].BuyPrice);
                            m_Credit = PlayerBank.instance.GetMoneyAmount();
                ...
                for (int i = 0; i < m_Categories.Length; i++)
                {
                    for (int x = 0; x < m_Categories[i].Items.Length; x++)
                    {
                        if (m_Categories[i].Items[x].BuildingPart.GetGeneralSettings.Identifier ==
                            buildingPart.GetGeneralSettings.Identifier)
                        {
                            PlayerBank.instance.AddMoney(m_Categories[i].Items[x].SellPrice);
                            m_Credit = PlayerBank.instance.GetMoneyAmount();
                        }
                    }
                }
        ...
        void Update()
        {
            m_CreditText.text = "Деньги: " + m_Credit;
        }
